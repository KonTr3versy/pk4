...
### MITRE ATT&CK (requires authentication)
| Method | Endpoint | Description |
|--------|----------|-------------|
| GET | `/api/attack/techniques` | Get all Enterprise techniques from MITRE |
| GET | `/api/attack/techniques/:id` | Get single technique by ID |
| GET | `/api/attack/tactics` | List all tactics with counts |
| GET | `/api/attack/status` | Check cache status |
| POST | `/api/attack/refresh` | Force refresh from MITRE TAXII |

**Query Parameters for `/api/attack/techniques`:**
- `tactic` - Filter by tactic (e.g., `Initial Access`)
- `platform` - Filter by platform (e.g., `Windows`)
- `search` - Search in ID, name, or description
